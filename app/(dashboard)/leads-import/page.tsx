...
import type { ResultadoImportacaoLeads } from '../../../lib/db/leads-import'
...
export default function LeadsImportPage() {
  ...
  const carregarHistorico = useCallback(async () => {
    const r = await fetch('/api/leads/importar').then(r => r.json())
  ...
  async function enviarArquivo(file: File) {
    ...
    try {
      ...
      const r = await fetch('/api/leads/importar', { method: 'POST', body: form })
    ...
    <div className="p-6 space-y-6 max-w-3xl">
      <div>
        <h1 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          Importar Leads (e-mail + UTM)
        </h1>
        <p className="text-xs text-gray-500 mt-0.5">
          Suba o export da Hotmart (.xlsx ou .csv, colunas email/utm_source/utm_medium/utm_campaign/utm_content/utm_term).
          O lançamento é detectado pelo prefixo da utm_campaign (ex: LC25_LEADS... → LC25). UTM já preenchida nunca é apagada.
        </p>
      </div>
      ...
      {resultado && (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4 space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            <Stat label="Linhas no arquivo" value={resultado.totalLinhas} />
        ...
        <div className="space-y-1.5">
          ...
          {historico.map((h, i) => (
            <div key={i} className="flex items-center justify-between text-xs border-b border-gray-800/60 pb-1.5">
              ...
              <span className="text-gray-500">{h.linhas} linhas</span>
...
function Stat({ label, value, highlight }: { label: string; value: number; highlight?: 'emerald' | 'blue' }) {
  const color = highlight === 'emerald' ? 'text-emerald-400' : highlight === 'blue' ? 'text-blue-400' : 'text-gray-200'
  return (
